package ru.agentlab.jfxed.diagramms.landscapeMap
...
import ru.agentlab.jfxed.figures.CommunicationPath.CommunicationPathFigure

public class LandscapeMap implements IDiagram {
	...
	override createJfx(OntModel jenaModel, XDiagram jfxDiagram) {
		...
		select ?uri 
		where { 
			?uri rdf:type <«SOURCE»#ApplicationComponent> 
		} 
		'''
		...
		for ( ; results.hasNext() ; ) {
	    	...
	    	val target = new CommunicationPathFigure() => [
				layoutX = 280
				layoutY = 280
				name = x.localName
			]
